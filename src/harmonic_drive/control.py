...
home_button = None
zero_button = None
SCANNER_UI_MIN_INTERVAL_S = 0.2
scanner_position_update_lock = threading.Lock()
scanner_position_latest = {
    "pos": None,
    "state": None,
    "machine_pos": None,
}
scanner_position_update_scheduled = False
scanner_position_last_apply = 0.0
...
def _run_on_ui_loop(callback, *, drop_reason: str = "no-ui-loop") -> bool:
    loop = ui_loop
    if loop is None or loop.is_closed():
        if threading.current_thread() is threading.main_thread():
            callback()
            return True
        logger.debug("Skipped UI callback because no live UI loop is available: {}", drop_reason)
        return False
    ...
    try:
        ...
        logger.debug("Skipped UI callback because the UI loop is unavailable: {}; {}", drop_reason, exc)
        return False


def _get_config_bool(section: str, key: str, fallback: bool = False) -> bool:
    config_file = scanner_app.config_file if scanner_app is not None else "config.ini"
...
def attach_gui_log_handler() -> None:
    ...
    if (
        not _get_config_bool("debug", "in_app_log_viewer", False)
        or log_handler is None
        or log_handler_sink_id is not None
    ):
        return
...
def _handle_measurement_progress(event: dict[str, Any]) -> None:
    ...
    _run_on_ui_loop(do_update, drop_reason="measurement-progress-callback")
...
def _apply_scanner_position_update(queued_at: float | None = None) -> None:
    global scanner_position_update_scheduled, scanner_position_last_apply
    started_at = time.monotonic()
    applied = False
    with scanner_position_update_lock:
        pos = scanner_position_latest["pos"]
        state = scanner_position_latest["state"]
        machine_pos = scanner_position_latest["machine_pos"]
        scanner_position_update_scheduled = False
        scanner_position_last_apply = started_at
    ...
    try:
        ...
        if "deleted" in str(exc).lower() or "slot" in str(exc).lower():
            reconnect_debug.record_scanner_ui_dropped(str(exc))
            return
        raise
    finally:
        if applied:
            reconnect_debug.record_scanner_ui_applied(
                queued_at,
                started_at,
                time.monotonic(),
            )
...
def update_scanner_position(pos=None, state=None, machine_pos=None):
    global scanner_position_update_scheduled
    reconnect_debug.record_scanner_update(state)
    loop = ui_loop
    if loop is None or loop.is_closed():
        reconnect_debug.record_scanner_ui_dropped("no live UI loop")
        return

    queued_at = None
    with scanner_position_update_lock:
        scanner_position_latest["pos"] = pos
        scanner_position_latest["state"] = state
        scanner_position_latest["machine_pos"] = machine_pos
        if scanner_position_update_scheduled:
            return
        now = time.monotonic()
        delay = max(
            0.0,
            SCANNER_UI_MIN_INTERVAL_S - (now - scanner_position_last_apply),
        )
        scanner_position_update_scheduled = True
        queued_at = reconnect_debug.record_scanner_ui_scheduled()

    def schedule_apply() -> None:
        if delay > 0:
            loop.call_later(delay, _apply_scanner_position_update, queued_at)
        else:
            _apply_scanner_position_update(queued_at)

    try:
        loop.call_soon_threadsafe(schedule_apply)
    except RuntimeError as exc:
        with scanner_position_update_lock:
            scanner_position_update_scheduled = False
        reconnect_debug.record_scanner_ui_dropped(str(exc))
...
def build_log_dialog():
    if not _get_config_bool("debug", "in_app_log_viewer", False):
        class DisabledLogDialog:
            def open(self):
                logger.info("In-app log viewer is disabled; use the terminal or log file.")

            def close(self):
                pass

        return DisabledLogDialog()
...
def build_control_pane(log_dialog):
    ...
    with ui.column().classes('w-full h-full min-w-0 overflow-auto px-2 pt-3 pb-2'):
        ...
        with ui.row().classes('items-center mt-1 gap-4'):
            if _get_config_bool("debug", "in_app_log_viewer", False):
                ui.button(
                    'Show Logs',
                    icon='list',
...
def initialize_app(config_file: str):
    ...
    reconnect_debug.set_deep_ui_diagnostics(
        _get_config_bool("debug", "deep_ui_diagnostics", False)
    )
